...
const LotteryCardContent = () => {
  ...
  const cakePrizesText = t('%cakePrizeInUsd% in Advar prizes this round', { cakePrizeInUsd: cakePriceBusdAsString })
  ...
  useEffect(() => {
    ...
    const fetchCurrentLotteryPrize = async () => {
      const { amountCollectedInWagyu } = await fetchLottery(lotteryId)
      const prizeInBusd = cakePriceBusd.times(amountCollectedInWagyu)
    ...
    <>
      <Flex flexDirection="column" mt="48px">
        ...
        <Text color="white" mb="40px">
          {t('Buy tickets with ADVAR, win ADVAR if your numbers match')}
        </Text>
      </Flex>
